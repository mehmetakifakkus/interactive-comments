...
- [x] First-level comments should be ordered by their score, whereas nested replies are ordered by time added.
- [x] Replying to a comment adds the new reply to the bottom of the nested replies within that comment.
- [x] A confirmation modal should pop up before a comment or reply is deleted.
- [x] Adding a new comment or reply uses the `currentUser` object from within the `data.json` file.
